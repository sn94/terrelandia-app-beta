...
//servicios
import {   getTotals, listarMisLoteamientos } from '../../api/Service';
...
export default function Index() {
    ...
    const { gastos_administrativos, comisiones_vendedores, otros_gastos } = requesting.loteamiento ?? {};
    const { total_cuotas_cobradas, intereses_cobrados, total } = requesting.loteamiento ?? {}
    ...
    return <View style={{ ...AppStyles.Container, justifyContent: "flex-start" }}>
       ...
       {  requesting.requesting && <ActivityIndicator size="large" color="#00ff00" />}
        ...
        <Liquidacion {...{ total_cuotas_cobradas, intereses_cobrados, total }} />
        <View style={{ ...AppStyles.row, justifyContent: "flex-start" }}>
            <Icon name="file-invoice-dollar" size={30} color={Colors.black}  ></Icon>
            <Text style={{ fontFamily: Fonts.normal, fontSize: 24, marginLeft: 15 }}>
        ...
        <Gastos {...{ gastos_administrativos, comisiones_vendedores, otros_gastos }}></Gastos>
    </View>
}